...
export default function Modal({
  ...
  useEffect(() => {
    ...
    clickContainer();
    featchCharacterData(info.name);
    mount = true;
  }, []);

  useEffect(() => {
    if (characterData) {
      setEquipment(characterData.equipment.items);
      setStasticsData(characterData.statistics.data);
    }
  }, [characterData]);
    ...
    },
    {
      ...
      effective: `${Math.round(
        ((stasticsData?.armor.effective ?? 0) /
          ((stasticsData?.armor.effective ?? 0) + 85 * info.level + 400)) *
          100
      )}%`,
    },
  ];
            ...
            <div id="캐릭터렌더,스텟" className=" w-[100%] h-[500px] mt-[10px]">
              ...
              >
                ...
                >
                  {""}
                  {[
                    { name: "힘", effective: stasticsData?.strength.effective },
                    {
                      name: "민첩성",
